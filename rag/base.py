    ...
    def generate_sparql(
        ...
        while True:
            ...
            if self.print_output:
                ...
                print("Generated SPARQL: ", response)
            try:
            ...
            if (len(result) == 0 or (len(result) == 1 and list(result[0].values())[0] == '0')) and try_threshold > 0:
                # failed
                try_threshold -= 1
                if verbose:
                    display(
...
DO NOT include any explanations or apologies in your responses. No pre-amble. Make sure to still answer using chain of thoughts and structure based on the format instruction defined in system prompt."""
    ...
    def run(
        ...
        #         fs["output"] = (
        #             "Thoughts: \n"
        #             + "\n".join(output["thoughts"])
        #             + "\n\nSPARQL: ```"
        #             + output["sparql"]
        #             + "```"
        #         )
        #     else:
        #         fs["output"] = f"SPARQL: ```{output['sparql']}```"
